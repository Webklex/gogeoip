...
CGO_ENABLED=1 GOOS=linux GOARCH=amd64 go build -a -ldflags "-linkmode external -extldflags '-static' -w -s -X main.buildNumber=${BUILD_NUMBER} -X main.buildVersion=${BUILD_VERSION}" -o ${OUTPUT_DIR}/${BINARY_NAME}
sleep 1
...
GOOS=windows GOARCH=amd64 go build -a -ldflags "-w -s -X main.buildNumber=${BUILD_NUMBER} -X main.buildVersion=${BUILD_VERSION}"  -o ${OUTPUT_DIR}/${BINARY_NAME}_x86.exe
sleep 1
...
GOOS=windows GOARCH=386 go build -a -ldflags "-w -s -X main.buildNumber=${BUILD_NUMBER} -X main.buildVersion=${BUILD_VERSION}"  -o ${OUTPUT_DIR}/${BINARY_NAME}_i686.exe
sleep 1
...
GOOS=darwin GOARCH=amd64 go build -a -ldflags "-w -s -X main.buildNumber=${BUILD_NUMBER} -X main.buildVersion=${BUILD_VERSION}" -o ${OUTPUT_DIR}/${BINARY_NAME}-darwin
sleep 1
